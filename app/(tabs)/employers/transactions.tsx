...
export default function EmployerTransactionsScreen() {
  ...
  const fetchTransactions = useCallback(async () => {
    try {
      ...
      // employerId가 있으면 해당 고용주의 거래만 가져오기
      if (employerId) {
        transactionRows = await transactionQueries.getByEmployerId(
          db,
          Number(employerId)
        );
      } else {
        // employerId가 없으면 모든 거래 가져오기
      ...
      // employerId가 있으면 해당 고용주 정보도 가져오기
      if (employerId) {
        const employerData = await employerQueries.getById(
          db,
          Number(employerId)
        );
        setCurrentEmployer(employerData || null);
      } else {
          ...
          <View style={styles.headerContent}>
            ...
            <View style={styles.headerTitleContainer}>
              <Text style={styles.headerTitle}>
                {currentEmployer
                  ? `${currentEmployer.name} 거래 기록`
                  : '거래 기록'}
              </Text>
              {currentEmployer && (
                <Text style={styles.headerSubtitle}>
                  {currentEmployer.tel &&
                    formatPhoneNumber(currentEmployer.tel)}
                </Text>
              )}
            </View>
