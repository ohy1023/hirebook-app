...
export default function EditWorkerScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await db.runAsync(
        `UPDATE workers SET 
         name = ?, tel = ?, note = ?, type = ?, birth_year = ?, 
         gender = ?, university = ?, uni_postcode = ?, uni_street = ?, 
         addr_postcode = ?, addr_street = ?, addr_extra = ?, nationality = ?, 
         face = ?, updated_date = ?
         WHERE id = ?`,
        [
          worker.name,
  ...
  useEffect(() => {
    navigation.setOptions({ headerTitle: '근로자 수정' });
  }, [navigation]);
